...
function JobPoller({ jobId, onDone }) {
  ...
  if (!job) return <div className="flex items-center gap-2 text-sm text-[#6B6660]"><Spinner className="w-4 h-4" /> Checking job status…</div>
  ...
  if (job.status === 'completed') {
    return (
      <div className="flex items-center gap-2 p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
        ...
        <div>
          <p className="text-sm text-green-600 font-medium">Ingest completed</p>
          <p className="text-xs text-[#6B6660]">{job.processed_records ?? 1} record(s) processed</p>
        </div>
  ...
  if (job.status === 'failed') {
    return (
      <div className="flex items-center gap-2 p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
        ...
        <div>
          <p className="text-sm text-red-600 font-medium">Ingest failed</p>
          {job.error_message && <p className="text-xs text-[#6B6660]">{job.error_message}</p>}
        </div>
    ...
    <div className="space-y-2">
      ...
      {pct !== null && (
        <div className="w-full bg-[#F0EEEA] rounded-full h-1.5">
          <div className="bg-[#0D9488] h-1.5 rounded-full transition-all" style={{ width: `${pct}%` }} />
        </div>
      )}
      <p className="text-xs text-[#8A8580] font-mono">Job ID: {jobId}</p>
    </div>
  )
}
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#6B6660] font-medium uppercase tracking-wider">Currency</label>
              <select
                value={form.currency}
                onChange={set('currency')}
                className="w-full bg-[#F0EEEA] border border-[#E8E5E0] rounded-md px-3 py-2 text-sm text-[#1B1A17] focus:outline-none focus:border-[#0D9488]/50"
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            />
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-[#6B6660] font-medium uppercase tracking-wider">Channel</label>
              <select
                value={form.channel}
