...
function getTrustLevel(score) {
  ...
  if (n >= 0.7) return { level: 'HIGH RISK', color: 'red', icon: '🛡️', bgClass: 'bg-red-500/20', borderClass: 'border-red-500/50', textClass: 'text-red-600' }
  if (n >= 0.4) return { level: 'FLAGGED', color: 'amber', icon: '⚠️', bgClass: 'bg-amber-500/20', borderClass: 'border-amber-500/50', textClass: 'text-amber-600' }
  return { level: 'VERIFIED', color: 'green', icon: '✓', bgClass: 'bg-green-500/20', borderClass: 'border-green-500/50', textClass: 'text-green-600' }
}
...
export function EntityTrustScoreCard({ entityId }) {
  ...
  if (isLoading) {
    return (
      <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-6 flex justify-center">
        <Spinner />
      </div>
    )
  }
  ...
  return (
    <div className={`bg-[#FFFFFF] border-2 ${trust.borderClass} rounded-lg p-6 mb-4`}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#8A8580] mt-1">Trust Score</p>
          </div>
        </div>
      </div>

      <div className="mb-4">
        <p className="text-xs text-[#8A8580] uppercase tracking-wider mb-2">Evidence Summary</p>
        <ul className="space-y-1.5">
          {evidencePoints.slice(0, 3).map((point, i) => (
            <li key={i} className="text-sm text-[#6B6660] flex items-start gap-2">
