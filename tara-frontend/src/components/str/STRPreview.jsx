...
function Field({ label, value, mono = true }) {
  return (
    <div>
      ...
      <p className={`text-sm text-[#1B1A17] break-all ${mono ? 'font-mono' : ''}`}>
        {value || '—'}
      </p>
    </div>
  )
  ...
  return (
    <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs text-[#8A8580] uppercase tracking-wider">Report Info</span>
      ...
      <div className="space-y-3">
        ...
        {str.reviewerNotes && (
          <div>
            <p className="text-[10px] text-[#8A8580] uppercase tracking-wider mb-0.5">Reviewer Notes</p>
            <p className="text-xs text-[#6B6660] leading-relaxed">{str.reviewerNotes}</p>
          </div>
        )}

        {str.payloadHash && (
          <div>
            <p className="text-[10px] text-[#8A8580] uppercase tracking-wider mb-0.5">Payload Hash</p>
            <p className="text-[10px] text-[#8A8580] font-mono break-all">{str.payloadHash}</p>
          </div>
        )}
        ...
        {str.squadRef && (
          <div className="flex items-center gap-2 p-2 bg-blue-500/10 border border-blue-500/20 rounded-md">
            ...
            <div className="min-w-0">
              <p className="text-[10px] text-blue-600 font-medium">Filed via Squad</p>
              <p className="text-[10px] text-[#8A8580] font-mono truncate">{str.squadRef}</p>
            </div>
          </div>
        )}
      </div>
