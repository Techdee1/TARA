...
export function SquadFeed() {
  ...
  return (
    <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg overflow-hidden">
      {toast && (
        <div className="px-4 py-2 text-xs font-medium flex items-center gap-2 bg-red-500/10 border-b border-red-500/30 text-red-600">
          <span>🚨</span>
          {toast}
        </div>
      )}

      <div className="px-4 py-3 border-b border-[#E8E5E0] flex items-center gap-3">
        <p className="text-xs text-[#8A8580] uppercase tracking-wider font-medium">Squad Live Feed</p>
        <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
        <div className="ml-auto">
          <button
            onClick={handleSimulate}
            disabled={isSimulating}
            className="flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-md bg-blue-500/10 border border-blue-500/30 text-blue-600 hover:bg-blue-500/20 transition-colors disabled:opacity-70 max-w-[320px] truncate"
          >
            {renderButtonLabel()}
          </button>
        </div>
      ...
      <div className="divide-y divide-[#E8E5E0] max-h-[400px] overflow-y-auto">
        {isLoading ? (
          <div className="flex justify-center py-8"><Spinner /></div>
        ) : isError ? (
          <div className="px-4 py-8 text-center">
            <p className="text-sm text-red-600">Feed unavailable. Check connection.</p>
          </div>
        ) : transactions.length === 0 ? (
          <div className="px-4 py-8 text-center">
            <p className="text-sm text-[#6B6660] mb-2">Waiting for Squad transactions.</p>
            <p className="text-xs text-[#8A8580]">Webhook is listening. Click Simulate to test.</p>
          </div>
        ) : (
          transactions.slice(0, 8).map((txn) => {
            ...
            return (
              <div key={txn.id} className={`px-4 py-3 border-l-4 ${borderColor} hover:bg-[#F0EEEA] transition-colors`}>
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <span className="text-sm text-[#1B1A17] font-medium truncate">{truncate(senderName, 20)}</span>
                    <span className="text-[#8A8580]">→</span>
                    <span className="text-sm text-[#1B1A17] truncate">{truncate(receiverName, 20)}</span>
                  </div>
                  {isSquadReal && (
                  ...
                  {isSquadSim && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-amber-500/15 border border-amber-500/30 ml-2 shrink-0">
                      ...
                      <span className="text-[9px] font-bold text-amber-600 tracking-wider">SIM</span>
                    </span>
                  )}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono text-[#0D9488]">{formatNaira(txn.amount ?? 0)}</span>
                  <span className="text-xs text-[#8A8580]">{getRelativeTime(timestamp)}</span>
                </div>
              </div>
            )
          })
