...
export function TransactionVolumeChart() {
  ...
  if (!alerts?.length) {
    return <div className="flex justify-center items-center h-[200px] text-[#8A8580] text-sm">No alerts detected yet</div>
  }
    ...
    <div className="h-[250px] grid grid-cols-[minmax(0,1fr)_156px] gap-3 items-center">
      <div className="relative h-full min-h-0">
        ...
        <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center">
          <span className="text-3xl font-semibold font-mono text-[#1B1A17] leading-none">{total}</span>
          <span className="text-[11px] text-[#8A8580] mt-1 tracking-wide uppercase">Alerts</span>
        </div>
      </div>
      <div className="space-y-2">
        {data.map((item) => (
          <div key={item.key} className="rounded-md border border-[#E8E5E0] bg-[#F0EEEA]/45 px-3 py-2.5">
            <div className="flex items-center justify-between gap-2 text-xs">
              <span className="inline-flex items-center gap-2 text-[#6B6660]">
                <span className="w-2 h-2 rounded-full" style={{ backgroundColor: item.fill }} />
                {item.name}
              </span>
              <span className="font-mono text-[#1B1A17]">{item.value}</span>
            </div>
          </div>
        ))}
      </div>
