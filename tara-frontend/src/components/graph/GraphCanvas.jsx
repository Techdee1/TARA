...
export function GraphCanvas({ nodes, links, onNodeClick, height }) {
  ...
  const render = useCallback(() => {
    ...
    // Precompute each node's direct neighbours so a hover can highlight its
    // connections at a glance — this is the whole point of a relationship
    // graph: seeing who a node is tied to, not just that it exists.
    const neighborsOf = new Map(nodeData.map((d) => [d.id, new Set()]))
    linkData.forEach((l) => {
    ...
    node
      .on('mouseover', (e, d) => {
        ...
        const neighbors = neighborsOf.get(d.id) ?? new Set()
        node.select('circle').attr('fill-opacity', (n) => (n.id === d.id || neighbors.has(n.id) ? 0.55 : 0.06))
        node.select('text').attr('opacity', (n) => (n.id === d.id || neighbors.has(n.id) ? 1 : 0.15))
        link
          .attr('stroke', (l) => (l.source.id === d.id || l.target.id === d.id ? '#0D9488' : '#2D3748'))
      ...
      .on('mouseout', (e) => {
        ...
        link.attr('stroke', '#2D3748').attr('stroke-opacity', 0.55)
