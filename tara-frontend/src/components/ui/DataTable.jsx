  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-[#E8E5E0]">
            ...
            <tr key={hg.id}>
              {hg.headers.map((header) => (
                <th
                  ...
                  className="px-4 py-3 text-left text-[10px] uppercase tracking-wider text-[#8A8580] font-medium cursor-pointer select-none hover:text-[#1B1A17] transition-colors"
                >
          ...
          {table.getRowModel().rows.map((row) => (
            <tr
              ...
              className={cn(
                ...
                onRowClick && 'cursor-pointer hover:bg-[#F0EEEA]',
                rowClassName?.(row.original)
              )}
            >
              {row.getVisibleCells().map((cell) => (
                <td key={cell.id} className="px-4 py-3 text-[#6B6660]">
      ...
      {table.getRowModel().rows.length === 0 && (
        <div className="py-12 text-center text-[#8A8580] text-sm">No records found</div>
      )}
    </div>
  )
}
