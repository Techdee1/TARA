...
const VolumeTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-[#F0EEEA] border border-[#E8E5E0] rounded-lg p-3 text-xs">
      <p className="text-[#6B6660] mb-1">{d.channel}</p>
...
const TimeTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-[#F0EEEA] border border-[#E8E5E0] rounded-lg p-3 text-xs">
      <p className="text-[#6B6660] mb-1">{label}</p>
      <p className="text-[#0D9488]">{payload[0].value} txns</p>
      <p className="text-[#6B6660]">{formatNairaShort(payload[1]?.value ?? 0)}</p>
    </div>
  )
}
  ...
  if (isLoading) {
    return (
      <div className="grid lg:grid-cols-2 gap-4">
        {[0, 1].map((i) => (
          <div key={i} className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-5 h-[260px] flex items-center justify-center">
            <Spinner />
          </div>
        ))}
      </div>
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-4">
      ...
      <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-5">
        <p className="text-xs text-[#8A8580] uppercase tracking-wider font-medium mb-4">
          Volume by Channel
        </p>
        {channelData.length === 0 ? (
          <div className="flex items-center justify-center h-[200px] text-[#8A8580] text-sm">No data</div>
        ) : (
