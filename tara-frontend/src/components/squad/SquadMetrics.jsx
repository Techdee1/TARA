...
function Tile({ label, value, sub, accent, isSquad }) {
  ...
  if (isSquad) {
    return (
      <motion.div
        ...
        className="relative bg-[#FFFFFF] border border-[#FF4C1D]/25 rounded-lg p-4 overflow-hidden"
      >
        ...
        <div className="relative">
          ...
          <p className="text-2xl font-semibold font-mono text-[#1B1A17]">{value}</p>
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      className={`bg-[#FFFFFF] border border-[#E8E5E0] border-l-2 ${border} rounded-lg p-4`}
    >
      <p className="text-[10px] text-[#8A8580] uppercase tracking-widest font-medium mb-2">{label}</p>
      <p className="text-2xl font-semibold font-mono text-[#1B1A17]">{value}</p>
      {sub && <p className="text-xs text-[#8A8580] mt-1">{sub}</p>}
    </motion.div>
  )
    ...
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-4 animate-pulse h-24" />
        ))}
      </div>
    )
  }
