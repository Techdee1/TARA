...
const CHANNEL_COLORS = {
  transfer: 'bg-[#F0EEEA] text-[#6B6660] border-[#E8E5E0]',
  pos:      'bg-amber-500/10 text-amber-600 border-amber-500/30',
...
function ChannelBadge({ channel }) {
  ...
  const cls = CHANNEL_COLORS[ch] ?? 'bg-[#F0EEEA] text-[#6B6660] border-[#E8E5E0]'
  return (
...
export function SquadActivityLog({ transactions, isLoading, onRowClick }) {
  ...
  const SortIcon = ({ field }) => {
    if (sortField !== field) return <span className="text-[#E8E5E0]">↕</span>
    return <span className="text-[#0D9488]">{sortDir === 'asc' ? '↑' : '↓'}</span>
  }

  return (
    <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg overflow-hidden">
      {/* Toolbar */}
      <div className="px-4 py-3 border-b border-[#E8E5E0] flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 flex-1 min-w-48">
          <MagnifyingGlassIcon className="w-4 h-4 text-[#8A8580] shrink-0" />
          <input
            type="text"
            placeholder="Search by name, reference, ID…"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="bg-transparent text-sm text-[#1B1A17] placeholder:text-[#8A8580] focus:outline-none w-full font-mono"
          />
        </div>
        <div className="flex items-center gap-2">
          <FunnelIcon className="w-3.5 h-3.5 text-[#8A8580]" />
          <select
            value={channelFilter}
            onChange={(e) => setChannelFilter(e.target.value)}
            className="bg-[#F0EEEA] border border-[#E8E5E0] rounded px-2 py-1 text-xs text-[#1B1A17] focus:outline-none focus:border-[#0D9488]/50"
          >
        ...
        <span className="text-xs text-[#8A8580] ml-auto">{filtered.length} records</span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center text-[#8A8580] text-sm">No transactions match your filters</div>
      ) : (
              ...
              <tr>
                {[
                  ...
                  <th
                    ...
                    className={cn(
                      'px-4 py-3 text-left text-[10px] uppercase tracking-wider text-[#8A8580] font-medium',
                      field && 'cursor-pointer hover:text-[#1B1A17] select-none'
                    )}
                  >
                  ...
                  <tr
                    ...
                    className={cn(
                      ...
                      isHighRisk
                        ...
                        : 'bg-[#FFFFFF] hover:bg-[#F0EEEA]'
                    )}
                  >
                    <td className="px-4 py-3 text-[#6B6660] font-mono text-xs whitespace-nowrap">
                      {formatDateTime(txn.date)}
                    </td>
                    <td className="px-4 py-3 text-xs max-w-[140px]">
                      <p className="text-[#1B1A17] truncate font-medium">
                        {truncate(txn.fromEntityName ?? txn.fromEntity)}
                      </p>
                      <p className="text-[#8A8580] font-mono text-[10px] truncate">{truncate(txn.fromEntity, 14)}</p>
                    </td>
                    <td className="px-4 py-3 text-xs max-w-[140px]">
                      <p className="text-[#1B1A17] truncate font-medium">
                        {truncate(txn.toEntityName ?? txn.toEntity)}
                      </p>
                      <p className="text-[#8A8580] font-mono text-[10px] truncate">{truncate(txn.toEntity, 14)}</p>
                    </td>
                    <td className="px-4 py-3 text-[#0D9488] font-mono text-xs whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 text-[#8A8580] font-mono text-[10px] max-w-[120px] truncate">
                      {txn.reference ?? '—'}
                    </td>
                  </tr>
                )
