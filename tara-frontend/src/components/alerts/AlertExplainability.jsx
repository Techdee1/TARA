...
export function AlertExplainability({ alert }) {
  ...
  return (
    <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-5 mb-4">
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs text-[#8A8580] uppercase tracking-wider font-medium">TARA AI Reasoning</p>
        {pattern && <SeverityPill severity={pattern.severity} />}
      </div>

      {/* Risk score breakdown */}
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-[#F0EEEA] rounded-lg p-3 text-center">
          <p className="text-2xl font-bold font-mono text-red-600">{riskPct}%</p>
          <p className="text-[10px] text-[#8A8580] mt-1 uppercase tracking-wider">Network Risk Score</p>
        </div>
        <div className="bg-[#F0EEEA] rounded-lg p-3 text-center">
          <p className="text-2xl font-bold font-mono text-[#1B1A17]">{entityCount}</p>
          <p className="text-[10px] text-[#8A8580] mt-1 uppercase tracking-wider">Entities in Ring</p>
        </div>
        <div className={`bg-[#F0EEEA] rounded-lg p-3 text-center ${anomalyFlag ? 'border border-amber-500/30' : ''}`}>
          <p className={`text-2xl font-bold font-mono ${anomalyFlag ? 'text-amber-600' : 'text-[#6B6660]'}`}>
            {anomalyFlag ? 'YES' : anomalyScore > 0 ? anomalyScore.toFixed(2) : '—'}
          </p>
          <p className="text-[10px] text-[#8A8580] mt-1 uppercase tracking-wider">Anomaly Flag</p>
        </div>
      </div>
      ...
      {pattern && (
        <div className="mb-4">
          <p className="text-xs text-[#8A8580] uppercase tracking-wider mb-2">Why TARA Flagged This</p>
          <p className="text-sm text-[#6B6660] mb-3">{pattern.summary}</p>
          <div className="space-y-1.5">
            {pattern.signals.map((signal, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="text-[#0D9488] text-xs mt-0.5 shrink-0">✓</span>
                <span className="text-xs text-[#6B6660]">{signal}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {reasonText && (
        <div className="mb-4 bg-[#F0EEEA] rounded-lg p-3 border-l-2 border-[#0D9488]">
          <p className="text-[10px] text-[#8A8580] uppercase tracking-wider mb-1.5">Detection Output</p>
          <p className="text-xs text-[#6B6660] font-mono leading-relaxed">{reasonText}</p>
        </div>
      )}

      {/* Model info */}
      <div className="flex flex-wrap gap-4 pt-3 border-t border-[#E8E5E0]">
        <div>
          <p className="text-[10px] text-[#8A8580] uppercase tracking-wider">Detection Model</p>
          <p className="text-xs text-[#1B1A17] font-mono mt-0.5">GNN Heuristic + Isolation Forest</p>
        </div>
        <div>
          <p className="text-[10px] text-[#8A8580] uppercase tracking-wider">Model Version</p>
          <p className="text-xs text-[#1B1A17] font-mono mt-0.5">heuristic_v1</p>
        </div>
        {pattern && (
          <div>
            <p className="text-[10px] text-[#8A8580] uppercase tracking-wider">Regulatory Reference</p>
            <p className="text-xs text-[#0D9488] font-mono mt-0.5">{pattern.regulatoryRef}</p>
          </div>
        )}
      </div>
    </div>
