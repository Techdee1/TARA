...
export default function STRDetail() {
  ...
  if (isError || !str) return (
    <div className="flex flex-col items-center py-16 gap-3">
      ...
      <p className="text-sm text-red-600">STR not found or failed to load</p>
      <button onClick={() => navigate('/str')} className="text-xs text-[#0D9488] hover:underline">
        ← Back to STR Reports
      </button>
    </div>
  )
    ...
    <div>
      ...
      {str.squadRef && (
        <div className="mb-4 flex items-center gap-3 p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
          ...
          <span className="text-sm text-green-600 font-medium">Filed via Squad</span>
          <span className="text-xs text-[#6B6660] font-mono truncate">{str.squadRef}</span>
        </div>
      )}
      ...
      {enforcement && (
        <div className="mb-4 rounded-lg border border-red-500/40 bg-red-500/10 overflow-hidden">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-red-500/20">
            ...
            <span className="text-sm font-semibold text-red-600">Enforcement Executed</span>
            <span className="ml-auto text-xs text-[#6B6660]">{enforcement.frozen_count} account{enforcement.frozen_count !== 1 ? 's' : ''} frozen · Funds transferred to compliance quarantine</span>
          </div>
          <div className="px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
            {enforcement.frozen_entities?.map((e) => (
              <div key={e.entity_id} className="flex items-center gap-2 bg-[#F0EEEA] rounded-md px-3 py-2">
                <span className="text-[10px] font-bold text-red-600 bg-red-500/20 rounded px-1.5 py-0.5 shrink-0">FROZEN</span>
                <div className="min-w-0">
                  <p className="text-xs text-[#1B1A17] truncate">{e.name}</p>
                  <p className="text-[10px] text-[#8A8580] font-mono truncate">{e.entity_id}</p>
                </div>
              </div>
            ))}
          </div>
          {enforcement.quarantine_entity_id && (
            <div className="px-4 py-2 bg-[#FFFFFF] border-t border-red-500/10 flex items-center gap-2">
              <span className="text-[10px] text-[#8A8580]">Quarantine account:</span>
              <span className="text-[10px] text-[#6B6660] font-mono">{enforcement.quarantine_entity_id}</span>
            </div>
          )}
        </div>
      )}
      ...
      {str.isPending ? (
        ...
        <div className="flex items-center gap-3 p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
          <span className="text-green-600 text-sm font-medium">✓ STR approved</span>
          <Button variant="outline" size="sm" onClick={handleDownloadPDF}>
            <Download size={14} />
            Download PDF
          </Button>
          <button onClick={() => navigate('/str')} className="ml-auto text-xs text-[#6B6660] hover:text-[#1B1A17]">
            Back to STRs →
          </button>
        </div>
      ) : (
        <div className="flex items-center gap-3 p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
          <span className="text-red-600 text-sm font-medium">✗ STR rejected</span>
          <Button variant="outline" size="sm" onClick={handleDownloadPDF}>
            <Download size={14} />
            Download PDF
          </Button>
      ...
      <Modal open={approveModal} onClose={() => !decisionLoading && setApproveModal(false)} title="Approve STR">
        <p className="text-sm text-[#6B6660] mb-3">
          You are approving and filing <strong className="text-[#1B1A17]">{str.id}</strong>. This will:
        </p>
        <ul className="text-sm text-[#6B6660] space-y-1 mb-4 list-none">
          <li className="flex items-center gap-2"><span className="text-red-600">•</span> Freeze all flagged accounts immediately</li>
          <li className="flex items-center gap-2"><span className="text-amber-600">•</span> Transfer funds to the TARA compliance quarantine account</li>
          <li className="flex items-center gap-2"><span className="text-[#0D9488]">•</span> Submit this STR to the NFIU audit trail</li>
        </ul>
        {str.payloadHash && (
          <div className="bg-[#F0EEEA] border border-[#E8E5E0] rounded-md p-3 mb-4">
            <p className="text-[10px] text-[#8A8580] font-mono break-all">Hash: {str.payloadHash}</p>
          </div>
        )}
      ...
      <Modal open={rejectModal} onClose={() => !decisionLoading && setRejectModal(false)} title="Reject STR">
        <p className="text-sm text-[#6B6660] mb-3">Provide a reason for rejecting this STR draft.</p>
        <textarea
          rows={3}
          value={rejectReason}
          onChange={(e) => setRejectReason(e.target.value)}
          placeholder="Rejection reason (required)…"
          className="w-full bg-[#F0EEEA] border border-[#E8E5E0] rounded-md p-3 text-sm text-[#1B1A17] placeholder:text-[#8A8580] focus:outline-none focus:border-[#0D9488]/50 resize-none mb-4"
        />
        <div className="flex gap-2 justify-end">
