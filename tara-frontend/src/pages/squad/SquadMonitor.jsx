...
export default function SquadMonitor() {
  ...
  return (
    <div>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="text-xl font-semibold text-[#1B1A17]">Monitor</h1>
            ...
            {lastUpdated && (
              <p className="text-[10px] text-[#8A8580] font-mono mt-0.5 hidden lg:block">{lastUpdated}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              disabled={refreshing}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-[#F0EEEA] border border-[#E8E5E0] text-[#6B6660] hover:text-[#1B1A17] hover:border-[#FF4C1D]/30 transition-colors disabled:opacity-50"
            >
      ...
      <div className="flex gap-1 mb-4 bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-1 w-fit">
        {TABS.map((tab) => (
          <button
            key={tab}
