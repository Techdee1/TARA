...
export default function AlertDetail() {
  ...
  if (!alert) return <div className="text-center py-16 text-[#8A8580]">Alert not found</div>
  ...
  return (
    <div>
      ...
      {/* Pattern + Score */}
      <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-4 mb-4 flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-[#1B1A17]">
            {PATTERN_LABELS[alert.patternType] ?? alert.patternType}
          </p>
          <p className="text-xs text-[#6B6660] mt-0.5">{alert.reason}</p>
          <p className="text-xs text-[#8A8580] mt-0.5">{alert.entityIds?.length ?? 0} entities involved</p>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold font-mono text-red-600">{(alert.riskScore * 100).toFixed(0)}%</p>
          <p className="text-xs text-[#8A8580]">Risk Score</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-4">
        <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg overflow-hidden">
          <div className="px-4 py-2 border-b border-[#E8E5E0]">
            <p className="text-xs text-[#8A8580] uppercase tracking-wider">Subgraph Visualization</p>
          </div>
          <GraphCanvas
            nodes={subNodes}
            links={subLinks}
        ...
        <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-4">
          <p className="text-xs text-[#8A8580] uppercase tracking-wider mb-3">Involved Entity IDs</p>
          <div className="space-y-1.5 overflow-y-auto max-h-72">
            {entityIds.length > 0 ? entityIds.map((eid) => (
              <button
                key={eid}
                onClick={() => navigate(`/entities/${eid}`)}
                className="block w-full text-left text-xs font-mono text-[#0D9488] hover:text-white bg-[#F0EEEA] hover:bg-[#E8E5E0] px-3 py-1.5 rounded transition-colors truncate"
              >
                {eid}
              </button>
            )) : (
              <p className="text-xs text-[#8A8580]">No entity IDs available</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-3 mb-4">
        <p className="text-[10px] text-[#8A8580] uppercase tracking-wider mb-1">Pattern Fingerprint</p>
        <p className="text-xs text-[#8A8580] font-mono break-all">{fingerprintHash}</p>
      </div>

      {/* STR Generation */}
      <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg p-4 mb-4">
        <p className="text-xs text-[#8A8580] uppercase tracking-wider mb-3">Generate Suspicious Transaction Report</p>
        <textarea
          value={reviewerNote}
          onChange={(e) => setReviewerNote(e.target.value)}
          placeholder="Optional reviewer notes for the AI (e.g. focus areas, context)..."
          rows={2}
          className="w-full bg-[#F0EEEA] border border-[#E8E5E0] rounded-md p-3 text-sm text-[#1B1A17] placeholder:text-[#8A8580] focus:outline-none focus:border-[#0D9488]/50 resize-none mb-3"
        />
      ...
      <Modal open={noteModal} onClose={() => setNoteModal(false)} title="Add Investigation Note">
        <div className="space-y-4">
          <textarea
            ...
            rows={4}
            className="w-full bg-[#F0EEEA] border border-[#E8E5E0] rounded-md p-3 text-sm text-[#1B1A17] placeholder:text-[#8A8580] focus:outline-none focus:border-[#0D9488]/50 resize-none"
          />
      ...
      <Modal open={dismissModal} onClose={() => setDismissModal(false)} title="Dismiss Alert">
        <p className="text-sm text-[#6B6660] mb-4">Are you sure you want to dismiss this alert? This action will be logged in the audit trail.</p>
        <div className="flex gap-2 justify-end">
