...
export default function EntityDetail() {
  ...
  if (!entity) return <div className="text-center py-16 text-[#8A8580]">Entity not found</div>
    ...
    <div className="space-y-4">
      ...
      {isFrozen && (
        <div className="flex items-center gap-3 px-4 py-3 bg-red-500/10 border border-red-500/40 rounded-lg">
          <span className="text-[10px] font-bold text-red-600 bg-red-500/20 rounded px-2 py-1">FROZEN</span>
          <div>
            <p className="text-sm font-semibold text-red-600">Account Frozen by TARA Compliance</p>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg overflow-hidden flex flex-col">
          <div className="px-4 py-2.5 border-b border-[#E8E5E0] shrink-0 flex items-center justify-between">
            <p className="text-xs text-[#8A8580] uppercase tracking-wider font-medium">
              1-Hop Network
            </p>
          ...
          {hopNodes.length > 1 ? (
            ...
            <div className="flex items-center justify-center h-40">
              <p className="text-xs text-[#8A8580]">No connected entities found</p>
            </div>
          )}
        </div>
      </div>
      ...
      {hasNeighbours && (
        <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg overflow-hidden">
          <div className="px-4 py-2.5 border-b border-[#E8E5E0]">
            <p className="text-xs text-[#8A8580] uppercase tracking-wider font-medium">Transaction Counterparties</p>
          </div>
              ...
              return (
                ...
                >
                  <div className="min-w-0 mr-4">
                    <p className="text-sm text-[#1B1A17] truncate">{n.full_name ?? n.canonicalName ?? nid}</p>
                    <p className="text-xs text-[#8A8580] font-mono truncate">{nid}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs font-mono text-[#0D9488]">{formatNairaShort(n.total_amount ?? n.totalAmount ?? 0)}</p>
                    <p className="text-[10px] text-[#8A8580]">{n.transaction_count ?? n.transactionCount ?? 0} txns · {n.relationship}</p>
                  </div>
                </button>
              )
            })}
      ...
      {(txLoading || hasTransactions) && (
        <div className="bg-[#FFFFFF] border border-[#E8E5E0] rounded-lg overflow-hidden">
          <div className="px-4 py-2.5 border-b border-[#E8E5E0] flex items-center justify-between">
            <p className="text-xs text-[#8A8580] uppercase tracking-wider font-medium">Transactions</p>
            {hasTransactions && <span className="text-xs text-[#8A8580]">{transactions.length} records</span>}
          </div>
          <div className="p-2">
      ...
      {hasAlerts && (
        <div>
          <p className="text-xs text-[#8A8580] uppercase tracking-wider mb-3">Linked Alerts</p>
